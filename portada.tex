\begin{titlepage}
	\centering
	

	\noindent
	\colorbox[RGB]{128,0,0}{%
		\parbox{\dimexpr\textwidth+1.5cm\relax}{\hspace{1cm}\rule{0pt}{1cm}}
	}
	\par\vspace{1 cm}

	\includegraphics[width=4cm]{logotiny.png}\par\vspace{1cm}
	

	{\Large \textbf{UNIVERSIDAD NACIONAL DEL }}\par 
	
		\vspace{0.1cm}
	
		{\Large \textbf{ALTIPLANO DE PUNO}}\par
	
		\vspace{0.4cm}
	
	{\large Dirección de Admisión}\par\vspace{0.5cm}
	

	{\Large \textbf{SOLUCIONARIO DE EXÁMENES DE }}\par
			
			\vspace{0.08cm}
			
			{\Large \textbf{ADMISIÓN}}\par
			
	\vspace{0.3cm}
	

	{\large Exámenes de Admisión 2024-I}\par
	
	{\large Examen GENERAL y Examen CEPREUNA}\par
	\vspace{1cm}
	

	\vfill
	\noindent
	
		{\large Puno – Perú}\par

	\colorbox[RGB]{230,230,230}{%
		\parbox{\dimexpr\textwidth+1.5cm\relax}{\hspace{1cm}\rule{0pt}{1.2cm}}
	}

	
\end{titlepage}
