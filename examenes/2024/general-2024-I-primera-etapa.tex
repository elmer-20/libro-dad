...
\begin{enumerate}
	...
	7(\overline{abcde}) &= 300,000 - 1 \\
	7(\overline{abcde}) &= 299,999 \\
	\overline{abcde} &= 42,857
	\end{align*}
	
	Piden $\overline{1abcde} $ \fbox{$= 142,857$}
	
	\vspace{6mm}
	
	\item Si $N$ es un número que descompuesto en sus factores primos es igual a $N = 2^5 \cdot 3^4 \cdot x$ y además $N$ es igual a $\dfrac{24}{77}$ de la suma de sus divisores, halle  la suma de dígitos de $x$.
...
\begin{enumerate}[resume]
	...
	\fbox{$V = \dfrac{s^{\dfrac{3}{2}}}{3\sqrt{6\pi}}$} \\
...
\begin{enumerate}[resume]
	...
	\fbox{$\sqrt{3} $}$= \tan 35^\circ + \tan 25^\circ + \sqrt{3} \tan 35^\circ \cdot \tan 25^\circ$
...
\begin{enumerate}[resume]
	...
	\item Dos esferas de igual volumen tienen pesos de 10 N y 50 N respectivamente, se encuentran en el agua flotando unidas mediante un resorte como se muestra en la figura. Halle la deformación del resorte. (K=5 N/cm)
	\begin{center}
	...
	$Q = \left( 1 \, \dfrac{cal}{g \cdot ^\circ C} \right) (1,000 \, g) (90 - 10) \, ^\circ C$
	...
	$t = \dfrac{8 \times 10^3 \times 4.2 \times 10}{420} =$\fbox{$ 800 \, s$}
...
$\ell = \dfrac{4}{24} =$\fbox{$ \dfrac{1}{6} \, m$}
...
\begin{enumerate}[resume]
	...
	\begin{itemize}
		...
		\item Adelanta: 720 min $\xrightarrow{\times 2}$ en 1,440 h.
	\end{itemize}
	
	Convertimos las horas a días:
	$1,440 \text{ h} = \dfrac{1,440}{24} = 60 \text{ días.}$
	
	$\Rightarrow$ Cada 60 días marcará la hora exacta.
	
	\vspace{2mm}
	\textbf{Para un año (365 días):}
	$\# \text{ veces} = \dfrac{365}{60} \approx $\fbox{$6 \text{ veces}$}
	...
	Reemplazando (II) en (I) y resolviendo:
	...
	De (III): $a + b + c + d = 124 + 4(4) =$\fbox{$ 140$}
...
\begin{center}
	\begin{minipage}{0.45\textwidth}
		...
		\hspace{20mm} \fbox{$= 4(\sqrt{2}\pi + 2\pi + 4)m$}
...
\begin{enumerate}[resume]
	...
	II presenta al personaje.
	III continúa cronológicamente con su juventud.
	I menciona sus descubrimientos y experimentos.
	IV habla del conflicto con la Inquisición.
	V concluye con “Por ello…”, consecuencia del punto IV.
...
\begin{enumerate}[resume]
	...
	\item Answer the question about the underlined word.
	"Mother's Day is \underline{in} May on the second Sunday."
...
\begin{enumerate}
	...
	\[\begin{array}{l}
		...
		\equiv [(p \vee ( \sim p \wedge q)) \wedge ( \sim q \vee ( \sim p \wedge q))] \vee  \sim (p \vee q)\\
		\equiv [(p \vee q) \wedge ( \sim q\, \vee  \sim p)] \vee  \sim (p \vee q)\\
		\equiv ( \sim q\, \vee  \sim p) \vee ( \sim p\, \wedge  \sim q)\\
		\equiv \boxed{\sim p\, \vee  \sim q}
	\end{array}\]
	...
	Donde $V_2$ + $V_4$ = \fbox{20\,L}
	
	\vspace{6mm}
	
	\item El dinero de A excede al de B en 20\% del dinero de C y el exceso de B a C equivale al 10\% del dinero de A. Si A tiene S/.200 ¿Cuántos tienen C y B juntos?
	...
	En(1) 200 - B =  $\dfrac{20}{100}$(150) = S/.170 = B
	
	A + B = 150 + 170 =\fbox{ 320}
...
\begin{enumerate}[resume]
	...
	\fbox{$\therefore {L_{perpendicular}} : 2x + 5y - 19 = 0$}
...
\begin{enumerate}[resume]
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=0.4\textwidth]{imagenes/general2024-I/etapa1/bio/trigonometria3g24-I}
		%\caption{Descripción de la imagen}
		%\label{fig:ejemplo}
	\end{figure}
	...
	\begin{minipage}[t]{0.35\textwidth}
		...
		$ \dfrac{{4}}{{sen\beta }}=\dfrac{{5}}{{sen\alpha }}=\dfrac{{6}}{{sen\theta }}=2R\\\\
		\dfrac{{sen\beta}}{{4}}=\dfrac{{sen\alpha}}{{5}}=\dfrac{{sen\theta}}{{6}}=\dfrac{{1}}{{2R}}$\\
		...
		$M = \dfrac{{\dfrac{5}{{2R}} + \dfrac{4}{{2R}}}}{{\dfrac{6}{{2R}}}} = \dfrac{{\dfrac{9}{{2R}}}}{{\dfrac{6}{{2R}}}} = \dfrac{9}{6} =$\fbox{$ \dfrac{3}{2}$}
	...
	\begin{minipage}[t]{0.65\textwidth}
		\vspace{0pt}
		\centering
		\includegraphics[width=0.7\textwidth]{imagenes/general2024-I/etapa1/bio/trigonometria3soluciong24-I}
	\end{minipage}
...
\begin{enumerate}[resume]
	...
	\begin{minipage}[t]{0.5\textwidth}
		...
		\left| {\overrightarrow R } \right| = $\fbox{$\sqrt {{{9x}^2}}=3x$}	
...
\begin{enumerate}[resume]
	...
	$E_{c}\,\text{después}=\dfrac{1}{2}m{\dfrac{4}{9}v}=\dfrac{2}{9}mv^2$
	
	$\dfrac{{{E_c}\,\text{antes}}}{{{E_c}\,\text{después}}} = \dfrac{{\dfrac{1}{2}m{v^2}}}{{\dfrac{2}{9}m{v^2}}} =$\fbox{$ \dfrac{9}{4}$}
...
\begin{enumerate}[resume]
	...
	$$ HC{I_{(ac)}} + NaO{H_{(ac)}} \to  NaC{L_{(ac)} + {H_2}{O_{(l)}} + calor }$$
...
\begin{enumerate}[resume]
	...
	CE = \dfrac{{200\,cal}}{{500 \rm{g}\times 5^\circ C}}=\dfrac{2}{25}$\\\\
...
\begin{enumerate}[resume]
	...
	\item Un gas está confinado en un cilindro de acero de $10.0$ litros, a una presión de 5 atmósferas y a $27^\circ$C. Si la temperatura se eleva a $327^\circ$C, ¿cuál será la presión del gas (en atm) en el interior del cilindro?
...
\begin{enumerate}[resume]
	...
	\item Juana recuerda una imagen que vio en el parque de diversiones. Esta información preliminar específica del cerebro corresponde a:
...
\begin{enumerate}[resume]
	...
	\item Entre 1347 y 1353, se produjo la pandemia más mortífera de la historia, conocida como $\_\_\_\_\_\_\_\_\_$. Los síntomas eran fiebre alta, dolores musculares, tos persistente, gangrena en dedos de las extremidades y aparición de enormes ampollas.
...
\begin{enumerate}[resume]
	...
	\item Pedro, al culminar sus estudios en el área de Biomédicas, elaborará un documento para que le emitan su certificado de estudios.\\
	¿Qué documento presentará Pedro a la coordinación académica de su escuela profesional?
...
\begin{enumerate}[resume]
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=5.8cm]{imagenes/general2024-I/etapa1/bio/razonamientomatematico2soluciong24-I}
	\end{figure}
...
\begin{enumerate}[resume]
	...
	$\therefore$ La fecha de nacimiento es \fbox{5 de julio}
...
\begin{enumerate}[resume]
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=5.5cm]{imagenes/general2024-I/etapa1/bio/razonamientomatematico4soluciong24-I.png}
	\end{figure}
...
\begin{enumerate}[resume]
	...
	Por eso, la oración que se debe eliminar es la \text{d)}
	...
	Por eso, el significado más próximo es:
	...
	Por lo tanto, el sinónimo correcto es \text{d) irrespirable}.
...
\begin{enumerate}[resume]
	...
	\item Which alternative is in the simple past tense?
	
	\begin{enumerate}[label=\alph*)]
		\item We worked in this hospital.
		\item We work in this hospital.
		\item We will work in this hospital.
	...
	El \textit{simple past tense} se utiliza para expresar acciones que ya ocurrieron y finalizaron en el pasado.
	
	En la oración \textit{We worked in this hospital}, el verbo \textit{worked} está en pasado, porque proviene del verbo base \textit{work}.
	
\end{enumerate}

\begin{enumerate}[resume]
	
	\item Which are milk-derived products?
	...
	Los \textit{milk products} o productos lácteos son alimentos derivados de la leche.
	
	Entre ellos se encuentran el queso, el yogur y la mantequilla.
...
\begin{enumerate}[resume]
	...
	La pregunta pide cuántos ojos tiene una persona. Una persona tiene dos ojos; en quechua se expresa como \textit{Iskay ñawikunayuq} y en aimara como \textit{paya nayranaka}. Por lo tanto, la alternativa correcta es la opción a.
...
\begin{enumerate}[resume]
	...
	La pregunta consulta cuántos huesos tiene el cuerpo humano. En el cuerpo de un adulto existen 206 huesos; en quechua se expresa como \textit{Iskay pachak suqtayuq tullukuna} y en aimara como \textit{pä pataka suxtani ch'akanaka}. Por lo tanto, la alternativa correcta es la opción \textbf{e)}
...
\begin{enumerate}[resume]
	...
	\begin{enumerate}[label=\alph*)]
		...
		\item Tiene razón y por ello puede actuar sin consciencia, pero sí con libertad.
...
\begin{enumerate}[resume]
	...
	\begin{center}
		\(
	\Rightarrow \overline{abcde}=40,404
		\)
	\end{center}
...
\begin{enumerate}[resume]
	...
	A: \rule{6cm}{0.4pt}\\
	B: No, they’re not accountants. They’re mathematicians.
	\begin{enumerate}[label=\alph*)]
		\item Are they accountants?
		\item Are you accountants?
		\item Is he an accountant?
		\item Are they mathematicians?
		\item Is he a mathematician?
	\end{enumerate}
	\textbf{Respuesta correcta:} a)
	...
	La pregunta correcta es: \textbf{Are they accountants?}, porque la respuesta indica: “No, they’re not accountants”, es decir, se está negando que ellos sean contadores.
	...
	La opción correcta es \textbf{Two thousand twenty three}, porque el número $2023$ en inglés se lee como “two thousand twenty three”.
...
\begin{enumerate}[resume]
	...
	La pregunta significa: \textbf{¿Cuántos días tiene un año?}
	
	Un año común tiene $365$ días.
	
	En quechua, $365$ se expresa como:
	\textbf{Kimsa pachaq suqta chunka pichqayuq p’unchaykuna}.
	
	En aimara, $365$ se expresa como:
	\textbf{kimsa pataka suxta tunka phisqani urunaka}.
	...
	La vaca come pasto o hierba, por lo tanto la respuesta es 
	
	\textbf{Q’achuta/Chuxlla}.
...
\begin{enumerate}
	...
	\item Las edades de Ana, Pedro y José suman 95 años y su producto es 27\,000. Además, la edad de Ana es a la de Pedro como la de Pedro es a la de José. ¿Cuál es la diferencia de edades entre Ana y José?
	\begin{enumerate}[label=\alph*)]
		\item 35
		\item 33
		\item 25
	...
	\item Carlos quiere comprar una chaqueta que cuesta \$200. El vendedor le ofrece un descuento del 15\% y un descuento adicional del 10\% por ser cliente frecuente. ¿Cuál es el descuento total que recibe Carlos?
	\begin{enumerate}[label=\alph*)]
		\item 29.5 \%
		\item 30.5 \%
		\item 23.5 \%
	...
	\item Al factorizar el polinomio $P(x,y)=-x^3y^2+xy^3$, se obtiene como factores $(ax+by)(cx^2+dy^2)$. Determina el valor de $a+b+c+d$.
	\begin{enumerate}[label=\alph*)]
		\item 2
		\item 5
		\item 3
	...
	\item Halla el valor de $m$ si el monomio $M(x)=-3\sqrt{2}\,x^{m-3}$ es de segundo grado.
	\begin{enumerate}[label=\alph*)]
		\item 2
		\item 6
		\item 5
	...
	\item En la figura, calcula el área de la región sombreada, si $AO=5\sqrt{10}$.
	
	\begin{center}
		\includegraphics[width=4.5cm]{imagenes/propuestos/geo5.png}
	\end{center}
	...
	\item De la figura mostrada, si $AC=7\sqrt{6}$, calcula $AD$.
	
	\begin{center}
		\includegraphics[width=4.5cm]{imagenes/propuestos/geo6.png}
	\end{center}
	...
	$\dfrac{\sen 3x+\sen x}{\cos 3x+\cos x}=n$
	
	determina el valor de:
	
	$E=\dfrac{1-\cos 4x}{1+\cos 4x}$
	...
	\item En un triángulo $ABC$, se sabe que:
	
	$\dfrac{a}{4}=\dfrac{b}{5}=\dfrac{c}{6}$
	
	Determine el coseno del menor ángulo interno.
	\begin{enumerate}[label=\alph*)]
		\item $\dfrac{1}{2}$
		\item $\dfrac{2}{3}$
		\item $\dfrac{3}{4}$
...
\item ¿Qué tiempo debe circular una corriente eléctrica de $10\text{ A}$ por una resistencia de $4.2\,\Omega$ para que con el calor disipado se logre elevar la temperatura de $480\text{ g}$ de agua de $40^\circ\text{C}$ a $50^\circ\text{C}$? ($1\text{ cal} = 4.2\text{ J}$).
...
\end{enumerate}
	...
	\item Un espejo cóncavo produce una imagen real de doble tamaño del objeto. Si la distancia del objeto a la imagen es de 30 cm, determine el radio de curvatura del espejo.
	\begin{enumerate}[label=\alph*)]
		\item 40 cm
		\item 60 cm
		\item 70 cm
	...
	\item ¿Cuántos enlaces $\sigma$ y $\pi$, respectivamente, hay en la hidrazina $(N_2H_4)$ y en el tetróxido de dinitrógeno $(N_2O_4)$?
	\begin{enumerate}[label=\alph*)]
		\item $5\sigma,\ 2\pi;\ 5\sigma,\ 0$
		\item $5\sigma,\ 0\pi;\ 5\sigma,\ 2\pi$
		\item $4\sigma,\ 1;\ 5\sigma,\ 2$
		\item $5\sigma,\ 4;\ 2,\ 3$
		\item $5\sigma,\ 0;\ 2,\ 5$
	\end{enumerate}
	
	\item La glucosa, que es uno de los compuestos de las aldosas, tiene en su molécula:
	\begin{enumerate}[label=\alph*)]
		\item El grupo carbonilo
		\item Moléculas de agua
		\item El grupo aldehído
		\item El grupo cetónico
		\item Un elemento metálico central
	\end{enumerate}
	
	\item Es la célula mesenquimatosa, embrionaria, indiferenciada y pluripotencial, capaz de dar origen a otras células del tejido conectivo:
	\begin{enumerate}[label=\alph*)]
		\item macrófago o histiocito.
		\item adipocito o célula adiposa.
		\item Stem-cell o célula madre.
		\item pericito o célula de Rouget.
		\item fibroblasto o célula de Unna.
	\end{enumerate}
	
	\item Una de las alternativas corresponde a las células blancas denominadas agranulocitos:
	\begin{enumerate}[label=\alph*)]
		\item Basófilo
		\item Neutrófilo
		\item Eosinófilo
	...
	\item Es la operación del pensamiento que permite establecer características comunes dentro de un conjunto de objetos:
	\begin{enumerate}[label=\alph*)]
		\item Análisis
		\item Síntesis
		\item Comparación
	...
	\item ¿Qué alternativa NO contiene un trastorno de la memoria?
	\begin{enumerate}[label=\alph*)]
		\item Paramnesia
		\item Hipermnesia
		\item Amnesia retrógrada
		\item Mnemotecnia
		\item Amnesia disociativa
	\end{enumerate}
	
	\item Las estrellas azules tienen un núcleo:
	\begin{enumerate}[label=\alph*)]
		\item Opaco ya que ha gastado la mitad del helio
		\item Brillante puesto que ha gastado poco hidrógeno
		\item Brillante puesto que ha gastado poco hidrógeno
		\item Gastado ya que han consumido casi todo el nitrógeno
		\item Muy brillante ya que está lleno de gases pesados
	\end{enumerate}
	
	\item Es la subcapa de la atmósfera terrestre que provoca que el cielo sea de color azul y se encuentra en:
	\begin{enumerate}[label=\alph*)]
		\item Sodiosfera - estratosfera
		\item Sodiosfera - mesosfera
		\item Ozono - mesosfera
		\item Ozono - estratosfera
		\item Ionosfera - mesosfera
	\end{enumerate}
	
	\item En esta fase se trata de aplicar todas las medidas planificadas y ensayadas con anterioridad para atenuar los daños personales.
	\begin{enumerate}[label=\alph*)]
		\item Prevención
		\item Emergencia
		\item Reconstrucción
	...
	\item En el Perú, entre los mecanismos de participación ciudadana previstos por la ley, aquel que involucra expresa y fundamentalmente a los llamados pueblos originarios --sin desmedro del empleo de los mecanismos restantes-- se denomina:
	\begin{enumerate}[label=\alph*)]
		\item revocatoria o remoción.
		\item iniciativa legislativa.
		\item referéndum.
		\item rendición de cuentas.
		\item consulta previa.
	\end{enumerate}
	
	\item Son obras de Julio Ramón Ribeyro:
	...
	\item En \textit{Redobles por Rancas}, Nictálope es sobrenombre de:
	\begin{enumerate}[label=\alph*)]
		\item Fortunato
		\item Rivera
		\item Córdova
	...
	\end{enumerate}
		...
		\item Una persona consume un determinado bien y se siente satisfecha. Corresponde a la característica de necesidades humanas:
	\begin{enumerate}[label=\alph*)]
		\item Son limitadas en número
		\item Varían en intensidad
		\item Tienden a fijarse
		\item Limitadas en capacidad
		\item Son concurrentes
	\end{enumerate}
	
	\item Las fases del ciclo económico son:
	\begin{enumerate}[label=\alph*)]
		\item Recuperación - Depresión - Auge – Recesión
		\item Recesión - Auge - Recuperación – Depresión
		\item Depresión - Auge - Recuperación – Recesión
		\item Depresión - Recuperación - Auge – Recesión
		\item Auge - Depresión - Recesión - Recuperación
	\end{enumerate}
	
	\item Son enunciados que corresponden a los planes políticos militares de San Martín, a excepción de:
	\begin{enumerate}[label=\alph*)]
		\item Aislar políticamente al virrey.
		\item Instaurar un gobierno que proteja los intereses de la monarquía criolla.
		\item Evitar enfrentamientos directos con las tropas del virrey.
		\item Establecer una monarquía constitucional de tipo borbónica.
		\item Ocupar la capital de manera violenta a través de una estrategia de guerra de guerrillas.
	\end{enumerate}
	
	\item Al término de la 2.a Guerra Mundial, EE.UU. dio su apoyo económico y militar a \dots, excepto:
	\begin{enumerate}[label=\alph*)]
		\item Hanoi
		\item Seúl
		\item Londres
	...
	\item Determina el número que falta en la siguiente distribución.
	
	\begin{center}
		\includegraphics[width=4.5cm]{imagenes/propuestos/rm5.png}
	\end{center}
	...
	\item Se quiere cercar el jardín mostrado en la figura, utilizando para ello 54 metros de cerca. Para calcular el área máxima de dicho jardín, determina $4x+y$.
	
	\begin{center}
		\includegraphics[width=4.5cm]{imagenes/propuestos/rm6.png}
	\end{center}
	...
	\item Elegir el término sinónimo de la palabra en mayúscula:
	
	Antonio se abandonó al vicio y a la \textbf{CONCUPISCENCIA}.
	...
	\item Según la lectura, los “primitivos” contemporáneos “cazan” hombres para conseguir sus cabezas porque:
	\begin{enumerate}[label=\alph*)]
		\item Necesitan conservarlas como adorno.
		\item Quieren apoderarse de algunos bienes del muerto.
		\item Creen que adquirirán poderes especiales.
		\item Es una virtud matar hombres e incorporarlos a sus trofeos.
		\item Aumentarán su prestigio social.
	\end{enumerate}
	
	\item “Ñawi” nisqaqa ima ch’akipim kachkan?
	\begin{enumerate}[label=\alph*)]
		\item Simi
		\item Chaki
		\item Ñawi
	...
	\item My house is \_\_\_\_\_ than yours.
	\begin{enumerate}[label=\alph*)]
		\item more big
		\item biggest
		\item bigger
	...
	\item How much \_\_\_\_\_ do you want in your coffee?
	\begin{enumerate}[label=\alph*)]
		\item sugar
		\item apples
		\item cookies
...
\begin{center}
	\begin{tabular}{ll ll ll ll ll ll}
		1) C & 2) C & 3) A & 4) C & 5) D & 6) E \\
		7) B & 8) C & 9) A & 10) A & 11) B & 12) C \\
		13) C & 14) D & 15) D & 16) D & 17) B & 18) D \\
		19) B & 20) E & 21) A & 22) D & 23) D & 24) D \\
		25) E & 26) A & 27) B & 28) D & 29) A & 30) C \\
		31) C & 32) C & 33) C & 34) A & & \\
	\end{tabular}
\end{center}
